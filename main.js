...
ipcMain.handle("get-loans-n", async () => {
    return new Promise((resolve, reject) => {
        db.all(
            "SELECT COUNT(*) n FROM loan WHERE returned = 0",
            [],
            (err, rows) => {
                if (err) {
                    reject(err);
...
ipcMain.handle("add-partner", async (event, partnerInfo) => {
    return new Promise((resolve, reject) => {
        db.run(
            "INSERT INTO partner (id, name, surname, grade, section, type) VALUES (?, ?, ?, ?, ?, ?)",
            [
                partnerInfo.id,
                partnerInfo.name,
                partnerInfo.surname,
...
ipcMain.handle("add-book", async (event, bookInfo) => {
    return new Promise((resolve, reject) => {
        db.run(
            "INSERT INTO book (id, inventory, title, author, edition, place, editorial, year, theme, borrowed, collection) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?)",
            [
                bookInfo.id,
                bookInfo.inventory,
                bookInfo.title,
...
ipcMain.handle("set-loan-state", async (event, id, returned) => {
    return new Promise((resolve, reject) => {
        db.run(
            "UPDATE loan SET returned = ? WHERE id = ?",
            [returned, id],
            function (err) {
                if (err) {
                    reject(err);
...
ipcMain.handle("update-loan", async (event, loanInfo) => {
    return new Promise((resolve, reject) => {
        db.run(
            "UPDATE laon SET date_start = ?, date_end = ?, book_id = ?, partner_id = ? WHERE id = ?",
            [
                loanInfo.date_start,
                loanInfo.date_end,
                loanInfo.book_id,
...
ipcMain.handle("get-loans", async () => {
    return new Promise((resolve, reject) => {
        db.all(
            "SELECT l.id, l.date_start, l.date_end, l.book_id, l.partner_id, l.returned, p.name, b.title, b.borrowed FROM loan l JOIN partner p ON l.partner_id = p.id JOIN book b ON b.id = l.book_id",
            [],
            (err, rows) => {
                if (err) {
                    reject(err);
...
ipcMain.handle("get-most-borrowed-books", async () => {
    return new Promise((resolve, reject) => {
        db.all(
            "SELECT b.id, b.title, b.author, COUNT(l.id) n_borrowed FROM book b JOIN loan l ON l.book_id = b.id GROUP BY b.id ORDER BY n_borrowed DESC LIMIT 3",
            [],
            (err, rows) => {
                if (err) {
                    reject(err);
...
ipcMain.handle("get-most-popular-themes", async () => {
    return new Promise((resolve, reject) => {
        db.all(
            "SELECT b.theme, COUNT(l.id) n_borrowed FROM book b JOIN loan l ON l.book_id = b.id GROUP BY b.theme ORDER BY n_borrowed DESC LIMIT 3",
            [],
            (err, rows) => {
                if (err) {
                    reject(err);
...
ipcMain.handle("get-most-reader-section", async () => {
    return new Promise((resolve, reject) => {
        db.all(
            "SELECT p.grade, p.section, COUNT(l.id) n_borrowed FROM partner p JOIN loan l ON l.partner_id = p.id GROUP BY p.grade, p.section ORDER BY n_borrowed DESC LIMIT 3",
            [],
            (err, rows) => {
                if (err) {
                    reject(err);
...
function addAuthorizedUser() {
    db.all("SELECT COUNT(*) n FROM user", [], (err, rows) => {
        if (err) {
            ...
            if (rows[0].n == 0) {
                db.run(
                    `INSERT INTO user (username, password) VALUES ("biblioteca", "$2a$10$ga61mawxeOR/1p7elh3Y4OjgQvbPtMZ.zozswJPBLLmjpGWLZJprW")`,
                    (err) => {
                        if (err) {
                            console.error(
                                "Error al insertar usuario autorizado:",
                                err.message
                            );
                        } else {
